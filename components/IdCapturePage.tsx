...
export default function IdCapturePage({
  ...
  successRecipient = "aelhassani@maunaloa.com.do",
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/send", {
        ...
        body: JSON.stringify({ pdfBase64, filename, clientName, senderKey }),
      });
      ...
      setStatus({
        type: "success",
        message: `Correo enviado a ${successRecipient}.`,
      });
    } catch {
      setStatus({
        type: "error",
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950 px-8 py-10 text-slate-100">
      ...
      <main className="mx-auto grid max-w-6xl grid-cols-1 gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
            ...
            <div className="flex items-center gap-2 rounded-full bg-slate-800/80 p-1 text-xs font-semibold text-slate-300">
              <button
                ...
                className={`rounded-full px-3 py-1 ${
                  captureSide === "front" ? "bg-slate-100 text-slate-900" : ""
                }`}
              ...
              <button
                type="button"
                className={`rounded-full px-3 py-1 ${
                  captureSide === "back" ? "bg-slate-100 text-slate-900" : ""
                }`}
        ...
        <section className="flex flex-col gap-6">
          ...
          <div className="rounded-3xl border border-slate-800/70 bg-slate-900/80 p-6 shadow-xl shadow-black/30">
            ...
            {!canGenerate && (
              <p className="mt-3 text-xs text-slate-400">
                Necesitas el nombre y ambas capturas para habilitar el PDF y el
                envio.
              </p>
            )}
          </div>
          ...
          <div
            className={`rounded-2xl border px-4 py-3 text-sm ${statusStyles[status.type]}`}
          >
            {status.message}
          </div>
        </section>
